...
export default function DoctorLoginPage() {
  async function loginDoctor(formData: FormData) {
    'use server'
    ...
    if (profile.role !== 'DOCTOR') {
      redirect(`/auth/error?message=Access denied: Not a doctor`)
    }
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#fcfdfe] px-6">
      <div className="w-full max-w-[400px]">
        ...
        <div className="text-center mb-10 space-y-2">
          ...
          <p className="text-slate-500 text-sm">Welcome back to your portal</p>
        </div>

        {/* Form Container */}
        <form 
          action={loginDoctor} 
          className="bg-white border border-slate-100 p-8 rounded-[24px] shadow-[0_20px_50px_rgba(0,0,0,0.02)] space-y-5"
        >
          ...
          <button className="group mt-2 w-full bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3.5 rounded-xl shadow-lg shadow-blue-100 flex items-center justify-center gap-2 transition-all active:scale-[0.98]">
            Sign In
          ...
          <div className="pt-4 flex flex-col items-center gap-2">
            <p className="text-sm text-slate-500">
              ...
              <Link href="/auth/doctor-sign-up" className="text-blue-600 font-medium hover:underline">
                Create one
              </Link>
            </p>
