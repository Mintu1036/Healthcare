...
export default function PatientLoginPage() {
  async function loginPatient(formData: FormData) {
    'use server'
    ...
    if (profile.role !== 'PATIENT') {
      redirect(`/auth/error?message=Access denied: Not a patient`)
    }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#fcfdfe] px-6">
      <div className="w-full max-w-[400px]">
        ...
        {/* Header matching the established style */}
        <div className="text-center mb-10 space-y-2">
          <h1 className="text-3xl font-semibold text-slate-800">
            Patient <span className="text-blue-600">Login</span>
          </h1>
          <p className="text-slate-500 text-sm">Access your personal health portal</p>
        </div>
        ...
        >
          ...
          <div className="pt-2 flex flex-col items-center gap-3">
            <p className="text-sm text-slate-500">
              New patient?{' '}
              <Link href="/auth/patient-sign-up" className="text-blue-600 font-medium hover:underline">
                Register here
              </Link>
            </p>
